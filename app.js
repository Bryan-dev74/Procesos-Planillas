(function(){
  // Animación de escritura con borrado y cursor para el header
  window.addEventListener('DOMContentLoaded', function() {
    var typeText = document.getElementById('typeText');
    var fullText = 'Procesos Planilla';
    var typeIdx = 0;
    var typing = true;
    function typeWriter(){
      if(!typeText) return;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if(stepImg && imgModal && modalImg){
    stepImg.addEventListener('click', function(){
      if(stepImg.src && !stepVisual.hidden){
        modalImg.src = stepImg.src;
